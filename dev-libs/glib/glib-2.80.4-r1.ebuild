...
multilib_src_configure() {
	...
	# Hack to use the local glib pc files since the ones from gobject-introspection subproject are not installed
	export PKG_CONFIG_PATH="${PWD}/meson-private:$(pkg-config --variable pc_path pkg-config)"
...
multilib_src_compile() {
	export LD_LIBRARY_PATH="$(echo ${PWD}/{glib,gobject,gthread,gmodule,gio,girepository} | tr ' ' ':'):${LD_LIBRARY_PATH}"
	env
	meson_src_compile
}
