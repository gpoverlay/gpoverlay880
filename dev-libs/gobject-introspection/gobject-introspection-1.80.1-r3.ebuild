...
GLIB_VERSION=2.80.4
GLIB_MAJOR=$(ver_cut 1 ${GLIB_VERSION})
# Taken from gnome.org eclass
if ver_test ${GLIB_VERSION} -ge 40.0; then
	: "${GLIB_RELEASE:=$(ver_cut 1 ${GLIB_VERSION})}"
else
	: "${GLIB_RELEASE:=$(ver_cut 1-2 ${GLIB_VERSION})}"
fi
SRC_URI="
	${SRC_URI}
	mirror://gnome/sources/glib/${GLIB_RELEASE}/glib-${GLIB_VERSION}.tar.${GNOME_TARBALL_SUFFIX}
"
...
src_prepare() {
	mv "${WORKDIR}/glib-${GLIB_VERSION}" "${S}/subprojects/glib"
	default
}
...
src_install() {
	meson_src_install --skip-subprojects glib
